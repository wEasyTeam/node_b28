...
exports.b28lib = function() {
    var coreVersion = "3.0",
        ...
        gettext = function(text, file) {
            var ret = MSG[text];
            if (!ret) {
                translatePageRemark.push(addInfo(text, '', 0, file));
            } else {
                return ret;
            }
        },
        ...
        content = content.replace(/((["'])(?:\\.|[^\\\n])*?\2)/g, function(key) {
            ...
            if (key !== '' && /[\u4e00-\u9fa5]/.test(key)) {
                if (MSG[key]) {
                    ...
                    translatePageRemark.push(addInfo(key, content, arguments[3], file))
                }
            }
    ...
    function addInfo(str, data, pos, file) {
        ...
        return (str + '\t:\t \t:\t 行号:' + row + '  文件:' + file + '\t:\thttp://127.0.0.1:8813/execute.html?execute://' + path.dirname(__dirname) + '\\execute\\notepad2.exe /r /g ' + row + ' ' + file + '\t:\t' +
    ...
    function GetResData(data, onlyZH, file) { //获取js等资源文件内语言
        ...
        var ignoreKeyWord = ['$(', 'getElementById(', 'find(', 'addClass(', '$.post(', '$.get(', 'delegate(', 'case ', 'hasClass(', 'indexOf(', 'getElementsByTagName(', 'getElementsByClassName(', 'on('];
